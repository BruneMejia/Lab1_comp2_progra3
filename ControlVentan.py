...
#función para agregar una venta a la lista
def venta():
    producto = entrada1.text()
    cantidad = entrada2.text()
    monto = entrada3.text()
# validacion, que los campos no esten vacios ya que si lo estan se muestra un mensaje de error
    if producto and cantidad and monto:
        try:
#Se valida que el monto sea un número válido, si no lo es se muestra un mensaje de error
#se convierte el monto a float para que se muestre con dos decimales
...
#muestra un mensaje de error si el monto no es un número válido
        ...
        QMessageBox.warning(ventana, "Error", "Por favor ingrese el productom, la cantidad y el monto.")
#función para eliminar una venta de la lista
...
#Creacion de la ventana principal de la aplicación
